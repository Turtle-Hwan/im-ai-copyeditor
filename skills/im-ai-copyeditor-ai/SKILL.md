...
**Phase 5** — `python3 $SKILL/scripts/reassemble.py _workspace/{run_id}/segments.json _workspace/{run_id}/worksheet.md --out _workspace/{run_id}/final.md --check-punctuation`. ID 중복·누락·추가, 빈 칸, 잘못된 `변경없음`은 2, 과윤문은 3, 보호 표현/행·열 변경 또는 문장부호 잔존은 4로 멈춘다. 칸 안의 문장 병합·분할과 의미 보존은 별도 검토한다. 실패하면 고친 뒤 다시 실행하며, 이전 결과를 이번 결과로 반환하지 않는다. 통과하려고 검사 옵션을 빼지 않는다.
