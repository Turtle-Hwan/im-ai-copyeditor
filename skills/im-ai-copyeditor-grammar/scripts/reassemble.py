...
def main(argv=None):
    ...
    for s in segments:
        ...
        if protected_values(s["core"], preserve) != protected_values(new_core, preserve):
        ...
        if s.get("role") and ("\n" in new_core or "\r" in new_core or
            ...
            return 4
        if args.check_punctuation and punctuation_issues(new_core, preserve):
    ...
    print(f"재조립 완료: {len(prose_ids)}개 텍스트 칸, 변경 {len(diffs)}건, 변경률 {change:.1%}")
