...
- 수치·고유명사·직접 인용·영어 약어·법령은 보존한다. 뜻이 모호한 부분만 유지하고 주변 표현은 계속 다듬는다.
- `변경없음` 칸을 마지막에 다시 읽는다. 맞춤법·번역투·군더더기·문장부호 교정이 빠졌으면 고친다. 변경률을 낮추려고 필요한 교정을 취소하지 않는다.
...
- `강도: 보수|기본|적극` — 기본값은 기본. 단계별 편집 범위는 공통 약속 5절을 따른다.
