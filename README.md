...
1. **분할.** `scripts/segment.py` 가 글을 문장 부호 기준으로 잘라 작업표를 만들어요.
   제목·목록·표는 구조 기호를 남기고 텍스트를 작업표에 넣어요. 코드 블록과 블록 인용은 그대로 둬요.
2. **윤문.** 에이전트가 작업표의 문장을 위에서 아래로 하나씩 읽고 고쳐요. 고칠 게 없으면 그대로 둬요.
3. **재조립.** `scripts/reassemble.py` 가 다듬은 문장을 원래 구조에 맞춰 다시 한 편으로 이어요.

재조립할 때 작업표 ID의 중복·누락·추가와 빈 윤문/규칙 칸을 검사해요. 변경이 없어도 원문과
`변경없음`을 적어야 해요. 변경률은 30%를 넘으면 의미 재검토를 권고하고, 기본 상한 50%를 넘으면 중단해요. 요청에 별도 상한이 있으면 `--max-change`로 명시해요. 검증에 실패해도
기존 결과 파일은 덮어쓰지 않아요. 칸 안의 문장 병합·분할과 의미 보존은 별도로 검토해야 해요.

통합 윤문과 AI 문체 명령은 중간점과 구분용 대시를 풀어 쓰고 `--check-punctuation`으로 잔존을 검사해요.
코드, 인용, URL, 숫자 범위, 인라인 수식은 예외예요. `AI·SW마에스트로`처럼 보존할 이름은 분절 전에
`--preserve-text 'AI·SW마에스트로'`로 지정해요. 문장부호 잔존이나 보호 표현 변경은 종료 코드 4로 멈춰요.

강조용 따옴표는 `--editable-quote '"정말"'`, 작성자의 안내문을 담은 인용 블록은 `--edit-blockquotes`로
다듬을 수 있어요. 실제 발언을 옮긴 인용문에는 적용하지 않아요. 자동 처리에서 빠진 인용과 HTML은
`부분 재조립`으로 알려 드려요. 스크립트 통과와 실제 교정 누락 검토는 별개예요.
맞춤법/문장 단독 명령은 사용자가 기호 제거를 요청할 때만 잔존 검사를 켜요.

구조 처리는 ATX 제목, 일반/체크/번호 목록 행, 파이프 표, 코드 펜스와 블록 인용을 대상으로 해요.
4칸 이상 들여쓴 줄은 코드로 보고 보존해요. 완전한 Markdown/HTML/MDX 파서가 아니므로 복잡한 중첩,
HTML/MDX 문서와 렌더링 결과는 별도로 검토해야 해요. 스크립트 없이 실행하는 환경에서는 자동 검사
완료로 표시하지 않고 수동 대조 결과와 한계를 알려요.
