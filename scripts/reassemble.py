#!/usr/bin/env python3
"""reassemble.py — 채운 워크시트를 원문 구조에 맞춰 한 편의 윤문본으로 되붙인다.

paraphrasing-korean 스킬의 강제 장치 2단계.
segments.json(원본 구조 정보) + worksheet.md(채운 윤문)를 합쳐 final.md 를 만든다.

핵심 안전장치:
  - 문장 수 검사: 작업표의 문장 칸 수 == 원본 문장 수.
    다르면(문장 누락/병합/추가) 멈춘다 → 뜻 보존이 깨진 것으로 본다.
  - 변경량 검사: 전체 변경량 30% 초과 경고, 50% 초과면 멈춘다.
  - '그대로 둘 줄'(헤딩·목록·코드 등)은 원문 그대로 지나간다.
